...
sudo su - deploy << EOF
  cat <<[-]EOF > /home/deploy/torresticket/backend/.env
NODE_ENV=
...
DB_DIALECT=postgres
DB_HOST=localhost
DB_USER=torresticket
DB_PASS=Ap10283012
DB_NAME=torresticket
DB_PORT=5432
DB_DEBUG=false
...
backend_start_pm2() {
  ...
  sudo su - deploy <<EOF
  cd /home/deploy/torresticket/backend
  pm2 start torresticket/server.js --name torresticket-backend
EOF
...
cat > /etc/nginx/sites-available/torresticket-backend << 'END'
server {
...
ln -s /etc/nginx/sites-available/torresticket-backend /etc/nginx/sites-enabled
EOF
